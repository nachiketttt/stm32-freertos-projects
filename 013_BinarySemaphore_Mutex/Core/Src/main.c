...
int main(void)
{
  ...
  if(xbinsema!=NULL)
  {
	  ...
	  status=xTaskCreate(Task1, "Task 1", 200, NULL, 2, &task1_handle);
	  configASSERT(status==pdPASS); //checking the returned value from the created task

	  //Employee task created
	  status=xTaskCreate(Task2, "Task 2", 200, NULL, 2, &task2_handle);
	  configASSERT(status==pdPASS); //checking the returned value from the created task

	  //making sema avail for the 1st time
...
static void Task1(void* parameters)
{
	const char* taskrun="Task 1 is running\n";
...
static void Task2(void* parameters)
{
	const char* taskrun="Task 2 is running\n";
	xSemaphoreTake(xbinsema, portMAX_DELAY);
	while(1)
	{
