...
{
	//here this variable allows the ISR to run the higher prio task available to CPU directly
	...
	//once the ISR exits this macro enable sthe higher prio task which got unblocked to resume on CPU
...
static void led_green_handler(void* parameters)
{
	...
	while(1)
	{
		...
		if(status==pdTRUE)
		{
			//this macro disables all interrupts above the configMAX_SYSCALL_INTERRUPT_PRIORITY currently set at 80 which coverts
			//to value 5 for stm any interrupt which has value below 5 still works perfectly fine
			portENTER_CRITICAL();
