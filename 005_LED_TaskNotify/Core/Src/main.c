...
static void led_green_handler(void* parameters)
{
	...
	while(1)
	{
		...
		if(status==pdTRUE)
		{
			...
			SEGGER_SYSVIEW_PrintfTarget("GREEN led task DELETED");
			vTaskDelete(NULL);
		}
	}
}
...
static void led_yellow_handler(void* parameters)
{
	...
	while(1)
	{
		...
		if(status==pdTRUE)
		{
			...
			SEGGER_SYSVIEW_PrintfTarget("YELLOW led task DELETED");
			vTaskDelete(NULL);
		}
	}
}
...
static void led_red_handler(void* parameters)
{
	...
	while(1)
	{
		...
		if(status==pdTRUE)
		{
			...
			SEGGER_SYSVIEW_PrintfTarget("Button task deleted");
			vTaskDelete(btn_task_handle);
			SEGGER_SYSVIEW_PrintfTarget("RED led task DELETED");
			vTaskDelete(NULL);
		}
	}
}
