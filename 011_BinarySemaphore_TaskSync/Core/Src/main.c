...
int main(void)
{
  ...
  if((xwork!=NULL) && (xworkqueue!=NULL))
  {
	  ...
	  status=xTaskCreate(ManagerTask, "Manager_task", 200, NULL, 4, &manager_task_handle);
	  configASSERT(status==pdPASS); //checking the returned value from the created task

	  //Employee task created
	  status=xTaskCreate(EmployeeTask, "Employee_task", 200, NULL, 2, &employee_task_handle);
	  configASSERT(status==pdPASS); //checking the returned value from the created task
...
static void ManagerTask(void* parameters)
{
	...
	//given using the xSemaphoreGive() API function before it can subsequently be taken (obtained)
	xSemaphoreGive(xwork);

	while(1)
	{
		//getting a random no. in ticket id
		ticketId=(rand() & 0x1FF);

		status=xQueueSend(xworkqueue, &ticketId, portMAX_DELAY);
		configASSERT(status==pdPASS);

		SEGGER_SYSVIEW_PrintfTarget("Ticket SENT to employee\n");

		//releasing the sempahore for the employee task to take it
		xSemaphoreGive(xwork);

		//after assigning the task yielding the processor
		taskYIELD();
	}
}
...
static void EmployeeTask(void* parameters)
{
	...
	while(1)
	{
		//employee task takes the semaphore if not avail it goes into blocking state it doesnt wait no delay asssigned
		xSemaphoreTake(xwork, 0);

		//get ticketid from manager task
		status=xQueueReceive(xworkqueue, &ticketId, 0);
		configASSERT(status==pdPASS);

		SEGGER_SYSVIEW_PrintfTarget("Ticket RECEIVED by employee\n");

		EmployeeDoWork(ticketId);
	}
}

void EmployeeDoWork(char ticketId)
{
	SEGGER_SYSVIEW_PrintfTarget("Employee task: Working on ticket %d\r\n",ticketId);
	vTaskDelay(ticketId);
}
/* USER CODE END 4 */
